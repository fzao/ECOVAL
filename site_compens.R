...
observeEvent(input$renseignerNH2,{
  ...
  if(length(rs) == 1){
    ...
    # update array visu CT
    tableau$C[pointer2row[rs],5] <- input$SCjustifCTNH
    if(is.null(input$SCdegincCTNH)) tableau$C[pointer2row[rs],6] <- ""
    else{
    ...
    tableau$C[pointer2row[rs],7] <- input$SCvalCTNH
    # update array visu LT
    tableau$C[pointer2row[rs],8] <- input$SCjustifLTNH
    if(is.null(input$SCdegincLTNH)) tableau$C[pointer2row[rs],9] <- ""
    else{
    ...
    tableau$C[pointer2row[rs],10] <- input$SCvalLTNH
    # save ecoval
    name <- paste("CC no.", input$selecthabitatSC)
...
observeEvent(input$renseignerNSP2,{
  ...
  if(length(rs) == 1){
    ...
    # update array visu CT
    tableau$D[pointer2row[rs],5] <- input$SCjustifCTNSP
    if(is.null(input$SCdegincCTNSP)) tableau$D[pointer2row[rs],6] <- ""
    else{
    ...
    tableau$D[pointer2row[rs],7] <- input$SCvalCTNSP
    # update array visu LT
    tableau$D[pointer2row[rs],8] <- input$SCjustifLTNSP
    if(is.null(input$SCdegincLTNSP)) tableau$D[pointer2row[rs],9] <- ""
    else{
    ...
    tableau$D[pointer2row[rs],10] <- input$SCvalLTNSP
    # save ecoval
    name <- paste("DC no.", input$selectspeciesSC)
