...
observeEvent(input$selecthabitatSC, {
  ...
  #cfz
  updateSelectInput(session, "selectspeciesSC", selected = "0")
})
...
observeEvent(input$renseignerNH2,{
  ...
  if(length(rs) == 1){
    ...
    # update array visu CT
    tableau$C[rs,5] <- input$SCjustifCTNH
    if(is.null(input$SCdegincCTNH)) tableau$C[rs,6] <- ""
    else{
    ...
    tableau$C[rs,7] <- input$SCvalCTNH
    # update array visu LT
    tableau$C[rs,8] <- input$SCjustifLTNH
    if(is.null(input$SCdegincLTNH)) tableau$C[rs,9] <- ""
    else{
    ...
    tableau$C[rs,10] <- input$SCvalLTNH
    # save ecoval
    name <- paste("CC no.", input$selecthabitatSC)
...
observeEvent(input$selectspeciesSC, {
  ...
  #cfz
  updateSelectInput(session, "selecthabitatSC", selected = "0")
})
...
observeEvent(input$renseignerNSP2,{
  ...
  if(length(rs) == 1){
    ...
    # update array visu CT
    tableau$D[rs,5] <- input$SCjustifCTNSP
    if(is.null(input$SCdegincCTNSP)) tableau$D[rs,6] <- ""
    else{
    ...
    tableau$D[rs,7] <- input$SCvalCTNSP
    # update array visu LT
    tableau$D[rs,8] <- input$SCjustifLTNSP
    if(is.null(input$SCdegincLTNSP)) tableau$D[rs,9] <- ""
    else{
    ...
    tableau$D[rs,10] <- input$SCvalLTNSP
    # save ecoval
    name <- paste("DC no.", input$selectspeciesSC)
