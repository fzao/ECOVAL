...
`r ecoval[[paste("Site no.", input$selectsiteimpact4)]][13,2]` km
...
```{r echo=FALSE, results='asis'}
    ...
    if(length(species) > 0){
      ...
      dat1 <- data.frame(ecoval[[name]][c(1,3:4,8:9), 1])
      colnames(dat1)[1] <- ""
      for(i in 1:length(species)){
        name <- paste("Espece", species[i])
        if(exists(name, where = ecoval)){
            dat1[[name]] <- ecoval[[name]][c(1,3:4,8:9), 2]
            dat1[[name]][2] <- as.character(A2listtype[dat1[[name]][2]])
            dat1[[name]][5] <- as.character(A2listprot[dat1[[name]][5]])
       ...
       for(i in 1:length(species)){
         ...
         if(exists(name, where = ecoval)){
             ...
             partial_select <- c(1,2,15,16,17,18,19)
              ...
              if(ecoval[[namesp]][4,2] == "2"){ # Communaute faunistique
...
`r ecoval[[paste("Site no.", input$selectsitecompens4)]][13,2]` km
...
```{r echo=FALSE, results='asis'}
    ...
    if(length(species) > 0){
      name <- paste("Espece", species[1])
      dat1 <- data.frame(ecoval[[name]][c(1,3:5,8:9), 1])
      colnames(dat1)[1] <- ""
      for(i in 1:length(species)){
        name <- paste("Espece", species[i])
        if(exists(name, where = ecoval)){
            dat1[[name]] <- ecoval[[name]][c(1,3:5,8:9), 2]
            if(dat1[[name]][4] == 1) dat1[[name]][4] = 'Oui'
            else dat1[[name]][4] = 'Non'
            dat1[[name]][2] <- as.character(A2listtype[dat1[[name]][2]])
            dat1[[name]][6] <- as.character(A2listprot[dat1[[name]][6]])
            colnames(dat1)[i+1] <- ecoval[[name]][2, 2]
        }
      }
      print(knitr::kable(dat1) %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = F, position = "left"))
